...
displayFolder.add(guiParams, 'linesCountA', 4, 36).step(2).name('球A線数').onChange((v) => {
  regenerateSphereA();
});
displayFolder.add(guiParams, 'longitudesCountB', 4, 36).step(2).name('球B経線数').onChange((v) => {
  regenerateSphereB();
});
displayFolder.add(guiParams, 'latitudesCountB', 4, 36).step(2).name('球B緯線数').onChange((v) => {
  regenerateSphereB();
});
displayFolder.open();
...
// 球Aを生成する関数
function generateSphereA(linesCount) {
  // 既存の線を削除
  while (ellipsesGroup.children.length > 0) {
    const child = ellipsesGroup.children[0];
    ellipsesGroup.remove(child);
...
// 球Bを生成する関数
function generateSphereB(longitudesCount, latitudesCount) {
  // 既存の線を削除
  while (latitudeLines.children.length > 0) {
    const child = latitudeLines.children[0];
    latitudeLines.remove(child);
    if (child.geometry) child.geometry.dispose();
  }

  // 経線を描画（垂直方向の円）
  for (let i = 0; i < longitudesCount; i++) {
    const longitude = createCircleLine(curveMaterial2);
    longitude.rotation.y = (Math.PI * i) / longitudesCount - Math.PI / 2;
  ...
  // 北半球と南半球の緯線
  const hemisphereDivisions = latitudesCount / 2 + 1;
...
// 球Bを再生成する関数
function regenerateSphereB() {
  const currentRotation = latitudeLines.rotation.clone();
  generateSphereB(guiParams.longitudesCountB, guiParams.latitudesCountB);
  latitudeLines.rotation.copy(currentRotation);
}
